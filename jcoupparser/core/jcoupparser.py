...
class JCoupFileMaker:
    ...
    def __init__(self, jcoupcsv_fname=None, jcoupwrite_fname=None):
        self.basestring = (
            "assign (resid   {0} and name hn )   (resid  {0} and name n ) "
    ...
    def read_jcoupcsv(self):
        ...
        with open(self.jcoupcsv_fname, "r") as csvfile:
            ...
            for i, row in enumerate(reader):
                ...
                self.read_file_lines.append(
                    (matched.group(1), int(matched.group(2)), float(strength))
                )
...
if __name__ == "__main__":
    jcoupmaker = JCoupFileMaker(
        jcoupcsv_fname="jcoup.csv", jcoupwrite_fname="jcoup.tbl"
    )
